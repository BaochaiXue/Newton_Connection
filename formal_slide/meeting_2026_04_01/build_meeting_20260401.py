...
RECALL_SLIDES: list[dict] = [
    {
        ...
        "subtitle": [
            ...
            "Opening = short recall first, then the completed rope apples-to-apples performance investigation.",
        ],
        "transcript": [
            "这一页只是 opening speaker page。",
            "这次 opening 先做很短的 recall，然后补一段已经完成的 rope apples-to-apples performance 结果。",
        ],
    },
    {
        "kind": "grid",
    ...
    {
        "kind": "body",
        "title": "Hypothesis P1: Rope Performance Must Be Established Apples To Apples Before Optimization",
        "bullets": [
            "**Fairness rule:** same rope case, same controller trajectory, same `dt/substeps`, same GPU, and no render.",
            "**Decision table first:** Newton **A0**, Newton **A1**, and PhysTwin **B0** answer the speed question.",
            "**Attribution second:** Newton **A2/A3** and PhysTwin **B1** explain the gap, but do not define throughput.",
            f"**Parity check passed:** PhysTwin vs IR controller trajectory max abs diff = **{B0_PAYLOAD.get('trajectory_parity', {}).get('controller_traj_max_abs_diff', 0.0):.1e}**.",
        ],
        "transcript": [
            ...
            "另外 same-case identity 这次不是口头假设，而是已经检查到 PhysTwin controller trajectory 和 IR 的 max abs diff 是 0，所以 benchmark 的输入本身是对上的。",
        ],
    },
    {
        "kind": "code_twocol",
        "title": "Source Proof: Newton Benchmarks No-Render Rope Replay, While PhysTwin Replays Through A Graph-Captured Path",
        "common_settings": "Left = Newton rope viewer already exposes no-render throughput vs attribution and baseline vs precomputed controller-write modes. Right = PhysTwin real-data simulator keeps `use_graph=True` and captures `forward_graph` for replay.",
        "left_label": "Newton rope replay benchmark: `--profile-only`, `throughput|attribution`, and `baseline|precomputed` controller-write are explicit.",
        "left_path": CODE_REPLAY_SEMANTICS_PNG,
        "right_label": "PhysTwin simulator path: `cfg.use_graph = True`, then `forward_graph` is captured once and launched during replay.",
    ...
    {
        "kind": "table",
        "title": "Result P1: Newton Is Still Slower On The Fair Rope Baseline",
    ...
    {
        "kind": "body",
        "title": "Result P2: Bridge Tax Explains Part Of The Gap, Not The Whole Gap",
        "bullets": [
            ...
            f"**Newton A3 split:** bridge **{A3_BRIDGE_MS:.3f}**, internal **{A3_INTERNAL_MS:.3f}**, integration **{A3_INTEGRATION_MS:.3f}**, collision **{A3_COLLISION_MS:.3f}**, unexplained **{A3_UNEXPLAINED_MS:.3f} ms/substep**.",
            f"**PhysTwin B1 split:** controller target **{B1_CONTROLLER_FRAME_MS:.3f} ms/frame**, simulator launch **{B1_SIM_LAUNCH_MS:.3f} ms/frame**, state reset **{B1_STATE_RESET_MS:.3f} ms/frame**.",
            "**Takeaway:** bridge tax matters, but the residual pure-rope gap is still execution structure, not collision.",
        ],
        "transcript": [
            ...
            f"而 PhysTwin B1 的 frame-level attribution 说明它的大头基本就是 simulator launch 本身，大约 `{B1_SIM_LAUNCH_MS:.3f} ms/frame`；controller target 和 state reset 都是小头。",
            "所以这页真正说服人的地方不是某一个数字，而是结构：controller bridge tax 确实存在，但 collision 几乎不占主因，残余 gap 仍然指向 runtime / execution structure。",
        ],
    },
    {
        "kind": "body",
        "title": "Result P3: Nsight Supports A Graph-Launch Explanation, Not A Collision Story",
        "bullets": [
            "**Newton A1 API:** **77.2% `cuLaunchKernel`**; the replay still looks like many decoupled launches.",
            "**PhysTwin B0 API:** **92.6% `cudaGraphLaunch`**; the replay is dominated by graph launch, not GUI work.",
            "**Interpretation:** the remaining pure-rope gap is consistent with launch structure / graph capture, not collision generation.",
            "**Only now optimize:** keep precomputed writes, then investigate graph-captured or more batched Newton replay before kernel micro-optimization.",
        ],
        "transcript": [
    ...
    },
    {
        ...
        "transcript": [
            "这里开始进入最终 self-collision campaign 的 6 页证据块。",
...
def _build_transcript_md() -> str:
    lines = [
        ...
        "形式：short recall + two completed new results  ",
        "目标：先用最短的 recall 回忆上周基线，再补 rope realtime viewer profiling 结果，以及 accepted bunny penetration visualization package",
        "",
        "---",
        "",
    ]
